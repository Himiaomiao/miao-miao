...
1、路由保护，客户端在访问需要登录的页面时，可以先使用中间件判断用户登陆状态，用户如果未登陆，则拦截请求，直接响应，禁止用户进入需要登录页面
...
| 属性：  | 描述：                                                       | 类型：   | 默认值：                   |
| ------- | ------------------------------------------------------------ | -------- | -------------------------- |
| inflate | 是否允许处理压缩的请求体，<br/>当设置为`false`时，压缩请求<br/>体的请求会被拒绝 | Boolean  | true                       |
| limit   | 控制请求体的大小，<br/>如果是一个数值，为指定<br/>的字节数，如果是一个字符串，该值会<br/>被传给[bytes](https://www.npmjs.com/package/bytes)库进行解析 | Mixed    | "100kb"                    |
| type    | 用于确定中间件将处理那种媒体类型，其值可<br/>以是一个字符串，字符串构成的数组或者一个<br/>函数，当不是一个函数时，该值会被传<br/>输给[`type-is`](https://www.npmjs.org/package/type-is#readme)库进行处理，该值还<br/>可以是一个拓展名（如`json`）,<br/>`mime type`(如`application/json`)或者包<br/>含通配符的 `mime type` （如`*/*` 或者 `*/json`），<br/>如果是一个函数，`type`类型将通过`fn(req)`调用<br/>，如果返回的是一个有效值，请求会被解析 | Mixed    | "application/octet-stream" |
| verify  | 如果这个选项被支持，将以<br/>`verify(req, res, buf, encoding)`形式被调用，<br/>其中`buf`是由原始请求体构成的`Buffer`,`encoding`<br/>是请求的编码，解析可以通过抛出错误而放弃 | Function | undefined                  |

### 2.2.3.`express.Router([options])`
...
你可以像对待`express`应用一样，给router添加中间件和各种方法。例如（`get`, `put`, `post`,）

### 2.2.4.`express.static(root,[options])`
...
注意：虽然这些中间件功能是通过一个特定的路由器添加的，但是当它们运行的时候是由它们所匹配的路径（而不是路由器）来定义的。因此，如果路由匹配，通过一个路由器添加的中间件可以运行其他路由器。例如，这段代码显示了两个不同的路由器匹配到了同一个路径
...
>+ 在项目中新建一个`mysql.js`文件，封装连接`mysql`数据库
>
>```javascript
>// 连接数据库
...
>    const { username, password } = { ...req.body };//获取客户端请求的数据信息
...
>    sqlQuery(sql, (data) => {
>      if (data.length == 0) {//先查询所要插入的数据是否已经存在
...
>    const sql = `select * from userinfo where username='${username}' AND password='${password}' `;
>    sqlQuery(sql, (data) => {
>      if (data.length != 0) {//先判断需要修改的数据是否存在
